...
# asyncio.create_subprocess_shell on Windows uses cmd.exe which doesn't
# understand Unix commands (ls, grep, cat, find, etc.). Git for Windows
# ships a full GNU environment. We detect it once at import time and use
# bash.exe -c "..." so the model's bash commands work identically to
# Linux/Mac — same as Claude Code. Falls back to cmd.exe if not found.
def _detect_git_bash() -> str:
    import platform
    if platform.system() != 'Windows':
        return ''  # Linux/Mac: create_subprocess_shell already uses /bin/bash
    candidates = [
    ...
    for path in candidates:
        if os.path.isfile(path):
            logger.info('Git Bash found at %s — bash tool will use GNU coreutils', path)
            return path
    import shutil
    found = shutil.which('bash')
    if found:
        logger.info('bash found on PATH at %s', found)
    else:
        logger.warning(
            'Git Bash not found — bash tool will use cmd.exe. '
            'Unix commands (ls, grep, cat) will not work. '
            'Install Git for Windows to fix this.'
        )
    return found or ''

_GIT_BASH_EXE: str = _detect_git_bash()

# ── Per-session persistent working directory ──────────────────────────────────
# Each bash invocation starts in the directory the previous one left off in,
# exactly like Claude Code's shell behavior. Without this, every `cd` is lost
# when the subprocess exits. Keyed by session_id; cleaned up after 4h of idle.
_session_cwd: Dict[str, str] = {}
_session_cwd_ts: Dict[str, float] = {}
_CWD_TTL_SECONDS = 4 * 3600  # 4 hours

_CWD_SENTINEL = "__ODYSSEUS_CWD__:"

def _get_session_cwd(session_id: Optional[str]) -> str:
    """Return the last known CWD for this session, or the home directory."""
    if not session_id:
        return os.path.expanduser('~')
    # Evict stale entries
    now = time.time()
    stale = [sid for sid, ts in _session_cwd_ts.items() if now - ts > _CWD_TTL_SECONDS]
    for sid in stale:
        _session_cwd.pop(sid, None)
        _session_cwd_ts.pop(sid, None)
    return _session_cwd.get(session_id, os.path.expanduser('~'))

def _set_session_cwd(session_id: Optional[str], cwd: str) -> None:
    if session_id and cwd:
        _session_cwd[session_id] = cwd
        _session_cwd_ts[session_id] = time.time()

def _wrap_bash_with_cwd(command: str, cwd: str) -> str:
    """Wrap a bash command to start in `cwd` and emit the final pwd as a sentinel."""
    # Escape the cwd for bash (handle spaces/special chars)
    safe_cwd = cwd.replace("'", "'\"'\"'")
    return f"cd '{safe_cwd}' 2>/dev/null || true\ntrap 'echo \"{_CWD_SENTINEL}$(pwd)\"' EXIT\n{command}"

def _extract_cwd_from_output(output: str) -> tuple[str, str]:
    """Split sentinel line from output. Returns (clean_output, new_cwd)."""
    ...
    return '\n'.join(clean_lines), new_cwd

def _normalize_path(path: str) -> str:
    """Normalize a path for Python's open() on Windows.

    Git Bash reports paths like /e/odysseus-main/ (MSYS2 Unix-style).
    Python's open() on Windows needs E:\\odysseus-main\\.
    This converter handles:
      /e/foo  -> E:\\foo      (single-letter drive, e.g. /e/ -> E:/)
      /c/foo  -> C:\\foo
      /tmp/   -> left as-is  (no Windows equivalent, will fail gracefully)
      E:\\foo -> E:\\foo      (already Windows, pass through)
      ~/foo   -> expanded    (handled by os.path.expanduser before us)
    """
    import platform
    if platform.system() != 'Windows':
        return path  # Linux/Mac: paths are fine as-is
    if not path:
        return path
    # Already a Windows path (has drive letter or UNC)
    if len(path) >= 2 and path[1] == ':':
        # Normalize forward slashes to backslashes (handles E:/foo/bar)
        return path.replace('/', '\\')
    if path.startswith('\\\\'):
        return path
    # Git Bash / MSYS2 style: /DRIVELETTER/rest
    # e.g. /e/odysseus-main -> E:\odysseus-main
    if len(path) >= 3 and path[0] == '/' and path[2] == '/' and path[1].isalpha():
        drive = path[1].upper()
        rest = path[3:].replace('/', '\\')
        return drive + ':\\' + rest
    # /DRIVELETTER only (e.g. /e)
...
async def _direct_fallback(
    ...
    try:
        if tool == "bash":
            # Resolve persistent CWD for this session (Claude Code-style shell state)
            _cwd = _get_session_cwd(session_id) if session_id else os.path.expanduser('~')
            _wrapped = _wrap_bash_with_cwd(content, _cwd)

            if _GIT_BASH_EXE:
                # Use Git Bash on Windows — gives full GNU coreutils (ls, grep,
                # cat, find, pwd, sed, awk, curl, etc.) so the model's bash
                # commands work identically to Linux/Mac (same as Claude Code).
                proc = await asyncio.create_subprocess_exec(
                    _GIT_BASH_EXE, '-c', _wrapped,
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                    env={**_subproc_env, 'MSYSTEM': 'MINGW64', 'CHERE_INVOKING': '1'},
                )
            else:
                # Fallback: cmd.exe (Unix commands won't work, but better than nothing)
            ...
            # Strip sentinel and persist new CWD
            stdout, new_cwd = _extract_cwd_from_output(stdout)
        ...
        if tool == "python":
            ...
            # Use the session's persistent CWD so Python matches bash.
            _py_cwd = _normalize_path(_get_session_cwd(session_id)) if session_id else None
            proc = await asyncio.create_subprocess_exec(
        ...
        if tool == "read_file":
            path = _normalize_path(os.path.expanduser(content.split("\n", 1)[0].strip()))
            if not path:
        ...
        if tool == "write_file":
            lines = content.split("\n", 1)
            path = _normalize_path(os.path.expanduser(lines[0].strip()))
...
async def execute_tool_block(
    ...
    if tool == "bash" and session_id:
        ...
        if _is_bg and _bg_cmd:
            ...
            _bg_cwd = _get_session_cwd(session_id)
            # Convert MSYS2 path for Popen's cwd (needs Windows path)
            _bg_cwd_win = _normalize_path(_bg_cwd) if _bg_cwd else None
            rec = bg_jobs.launch(_bg_cmd, session_id=session_id, cwd=_bg_cwd_win)

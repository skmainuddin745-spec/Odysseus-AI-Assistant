...
# Pattern 6: Llama 3.3/3.1 native tool calls leaked by local endpoints (LM Studio, etc)
...
def parse_tool_blocks(text: str) -> List[ToolBlock]:
    ...
    6. Llama 3.3 `<function:name{args}></function>` syntax
    """
    blocks = []
    ...
    # Pattern 6: Llama 3.3 native tags
    if not blocks:
        for m in _FUNCTION_TAG_RE.finditer(text):
            tool_name = m.group(1).lower()
            args_json = m.group(2).strip()
            from src.tool_schemas import function_call_to_tool_block
            block = function_call_to_tool_block(tool_name, args_json)
            if block:
                blocks.append(block)

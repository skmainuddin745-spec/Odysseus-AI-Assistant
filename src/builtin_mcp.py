...
async def _is_npx_package_cached(npx_path, package_spec, timeout_s=8):
    """Probe whether an npx package is already in the local cache.

    Modern npx (npm v10+, Node.js v18+) removed --no-install. We try
    two strategies:
      1. `npx --prefer-offline <pkg> --version` (modern npx)
      2. `npx --no-install <pkg> --version` (legacy npx v6)
    We treat "exited 0 with non-empty stdout" as proof of a working
    cached copy. Anything else means we should skip the server.
    """
    for flag in ("--prefer-offline", "--no-install"):
        try:
            proc = await asyncio.create_subprocess_exec(
                npx_path, flag, package_spec, "--version",

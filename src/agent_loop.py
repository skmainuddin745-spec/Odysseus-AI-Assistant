...
- Multiple tool blocks per response OK. Up to 1h timeout per tool, 10K char output limit.
...
ENVIRONMENT: On Windows, commands run in Git Bash (GNU coreutils) — use standard Unix commands (ls, grep, cat, find, pwd, sed, awk, curl, wc, sort, etc.), NOT Windows commands (dir, type, findstr). On Linux/Mac, native bash.
PERSISTENT WORKING DIRECTORY: `cd` persists across calls — if you `cd /some/path` in one bash call, the next bash call starts there automatically (like a real terminal). You do NOT need to re-cd every time.
PATH COMPATIBILITY: If you get a Unix-style path from bash (e.g. `/e/odysseus-main/app.py`), you can pass it directly to `read_file` or `write_file` — it will be auto-converted to the Windows equivalent.
...
_TOOL_SELECTION_TIMEOUT_SECONDS = 3.0
...
def _build_system_prompt(
    ...
    try:
        ...
        # Use cross-platform format codes: %d and %I work on Windows.
        # lstrip('0') removes the leading zero without using %-d / %-I
        # which are POSIX-only and crash on Windows with ValueError.
        _day = _now.strftime('%d').lstrip('0') or '0'
        _hour = _now.strftime('%I').lstrip('0') or '0'
        _ampm = _now.strftime('%p')
        agent_prompt = (
            f"## Current date and time\n"
            f"Today is {_now.strftime('%A, %B')} {_day}, {_now.strftime('%Y')} ({_now.strftime('%Y-%m-%d')}). "
            f"Local time is {_hour}:{_now.strftime('%M')} {_ampm} ({_now.strftime('%Z')}, UTC{_off_fmt}); "
...
async def stream_agent_loop(
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        # PARALLEL EXECUTION: Independent, side-effect-free tools (read_file,
        # web_search, web_fetch) can safely run concurrently — like Claude Code.
        # Side-effectful tools (bash, python, write_file, document tools) run
        # serially because they have ordering dependencies.
        _PARALLELIZABLE_TOOLS = frozenset({
            "read_file", "web_search", "web_fetch",
        })

        def _can_parallelize_batch(blocks):
            """Check if a consecutive batch of tool blocks can run in parallel."""
            if len(blocks) < 2:
                return False
            return all(b.tool_type in _PARALLELIZABLE_TOOLS for b in blocks)

        # Group consecutive parallelizable tools into batches
        _tool_batches = []
        _current_batch = []
        for block in tool_blocks:
            if block.tool_type in _PARALLELIZABLE_TOOLS:
                _current_batch.append(block)
            else:
                if _current_batch:
                    _tool_batches.append(("parallel" if len(_current_batch) > 1 else "serial", _current_batch))
                    _current_batch = []
                _tool_batches.append(("serial", [block]))
        if _current_batch:
            _tool_batches.append(("parallel" if len(_current_batch) > 1 else "serial", _current_batch))
        ...
        budget_hit = False
        # --- PARALLEL FAST PATH ---
        # When ALL tool blocks in this round are parallelizable, run them
        # concurrently with asyncio.gather for ~Nx speedup.
        _all_parallelizable = (
            len(tool_blocks) > 1
            and all(b.tool_type in _PARALLELIZABLE_TOOLS for b in tool_blocks)
            and (max_tool_calls <= 0 or total_tool_calls + len(tool_blocks) <= max_tool_calls)
        )

        if _all_parallelizable:
            ...
            async def _exec_parallel(blk):
                return await execute_tool_block(
                    blk, session_id=session_id,
                    disabled_tools=disabled_tools, owner=owner,
                )

            _parallel_results = await asyncio.gather(
                *[_exec_parallel(b) for b in tool_blocks],
                return_exceptions=True,
            )

            # Process results in order
            for block, raw in zip(tool_blocks, _parallel_results):
                total_tool_calls += 1
                cmd_display = block.content.strip()
                if isinstance(raw, Exception):
                    desc = f"{block.tool_type}: error"
                    result = {"error": str(raw), "exit_code": 1}
                else:
                    desc, result = raw

                # Extract web sources
                _src_text = result.get("output") or result.get("results") or ""
                ...
                tool_event = {
                    ...
                    "command": cmd_display, "output": output_text,
            ...
            logger.info(f"Parallel execution: {len(tool_blocks)} tools in round {round_num}")

        else:
            # --- SERIAL PATH (original) ---
            pass  # fall through to serial for loop below
